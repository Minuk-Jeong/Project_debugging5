                                  ...
                                  NR_UE_UL_BWP_t *current_UL_BWP)
{
  ...
  if (pusch_Config && pusch_Config->uci_OnPUSCH) {
    ...
    if (onPusch &&
      ...
      // Multiplex periodic CSI if present and no aperiodic CSI is already scheduled
      if (pucch->n_csi > 0 && pusch_pdu->pusch_uci.csi_part1_bit_length == 0 && pusch_pdu->pusch_uci.csi_part2_bit_length == 0) {
        pusch_pdu->pusch_uci.csi_part1_bit_length = pucch->n_csi;
        pusch_pdu->pusch_uci.csi_part1_payload = pucch->csi_part1_payload;
        pusch_pdu->pusch_uci.csi_part2_bit_length = 0;
        pusch_pdu->pusch_uci.csi_part2_payload = 0;
        pusch_pdu->pusch_uci.beta_offset_csi1 = pusch_pdu->pusch_uci.csi_part1_bit_length < 12 ?
                                               *beta_offsets->betaOffsetCSI_Part1_Index1 :
                                               *beta_offsets->betaOffsetCSI_Part1_Index2;
        pusch_pdu->pusch_uci.beta_offset_csi2 = 0; // No part2 for periodic CSI
        mux_done = true;
      }
      
      // Set alpha_scaling if any UCI is multiplexed
      if (mux_done) {
    ...
    // If periodic CSI is present but UCI on PUSCH is not configured, skip multiplexing
    if (pucch->n_csi > 0 && pusch_pdu->pusch_uci.csi_part1_bit_length == 0 && pusch_pdu->pusch_uci.csi_part2_bit_length == 0) {
      LOG_E(NR_MAC, "Periodic CSI present but UCI on PUSCH not configured, sending via PUCCH\n");
    }
  }
