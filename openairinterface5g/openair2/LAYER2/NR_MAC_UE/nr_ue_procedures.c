...
#ifdef RTD_MATCH_LOG
/*! \fn void get_wallclock_str(char *buf, size_t len);
   \brief Get wall-clock time string in HH:MM:SS.mmm format (thread-safe)
   \param buf Output buffer for time string
   \param len Size of output buffer
*/
static void get_wallclock_str(char *buf, size_t len)
{
  struct timespec rt;
  if (clock_gettime(CLOCK_REALTIME, &rt) == 0) {
    ...
    snprintf(buf, len, "%02d:%02d:%02d.%03d", tm.tm_hour, tm.tm_min, tm.tm_sec, ms);
  } else {
    snprintf(buf, len, "00:00:00.000");
  }
}

#ifdef RTD_MATCH_LOG_ACK
/*! \fn void get_bitstring_msb(char *buf, size_t len, uint64_t val, int nbits);
   \brief Get bitstring in MSB-first format (bit N-1 .. 0)
   \param buf Output buffer
   \param len Size of output buffer
   \param val Value to convert
   \param nbits Number of bits
*/
static void get_bitstring_msb(char *buf, size_t len, uint64_t val, int nbits)
{
  if (nbits <= 0 || len < (size_t)(nbits + 1)) {
    snprintf(buf, len, "(none)");
    return;
  }
  for (int i = nbits - 1; i >= 0; i--) {
    buf[nbits - 1 - i] = ((val >> i) & 1) ? '1' : '0';
  }
  buf[nbits] = '\0';
}

/*! \fn void get_bitstring_lsb(char *buf, size_t len, uint64_t val, int nbits);
   \brief Get bitstring in LSB-first format (bit 0 .. N-1)
   \param buf Output buffer
   \param len Size of output buffer
   \param val Value to convert
   \param nbits Number of bits
*/
static void get_bitstring_lsb(char *buf, size_t len, uint64_t val, int nbits)
{
  if (nbits <= 0 || len < (size_t)(nbits + 1)) {
    snprintf(buf, len, "(none)");
    return;
      ...
      case NR_PUCCH_Resource__format_PR_format2 :
        ...
        // RTD_MATCH: Log final PUCCH payload assembly with timestamp and frame/slot info
        {
          ...
          if (clock_gettime(CLOCK_MONOTONIC, &ts) == 0) {
            ...
            int numerology = mac->frame_structure.numb_slots_frame == 10 ? 0 : 
                             (mac->frame_structure.numb_slots_frame == 20 ? 1 : 
                             (mac->frame_structure.numb_slots_frame == 40 ? 2 : 3));
            int subframe = slot / (mac->frame_structure.numb_slots_frame / 10);
            int total_bits = pucch->n_csi + pucch->n_harq + pucch->n_sr;
            int csi_offset = pucch->n_harq + pucch->n_sr;
            int csi_msb = total_bits - 1;
            int csi_lsb = total_bits - pucch->n_csi;
            
            // Find PUCCH resource ID from list
            int pucch_res_id = -1;
            if (pucch_Config && pucch_Config->resourceToAddModList) {
              for (int i = 0; i < pucch_Config->resourceToAddModList->list.count; i++) {
                if (pucch_Config->resourceToAddModList->list.array[i] == pucchres) {
                  pucch_res_id = pucch_Config->resourceToAddModList->list.array[i]->pucch_ResourceId;
                  break;
                }
              }
            }
            
            // Generate bitstring for final payload (MSB->LSB)
            char bits_msb[128] = {0};
            for (int i = total_bits - 1; i >= 0; i--) {
              bits_msb[total_bits - 1 - i] = ((pucch_pdu->payload >> i) & 1) ? '1' : '0';
            }
            
            // Generate bitstring for final payload (LSB->MSB)
            char bits_lsb[128] = {0};
            for (int i = 0; i < total_bits; i++) {
              bits_lsb[i] = ((pucch_pdu->payload >> i) & 1) ? '1' : '0';
            }
            
            // Extract CSI bits from final payload
            uint64_t csi_extracted = (pucch_pdu->payload >> csi_offset) & ((1UL << pucch->n_csi) - 1);
            
            // Main matching log line (one line for grep)
            char wall[32];
            get_wallclock_str(wall, sizeof(wall));
            
#ifdef RTD_MATCH_LOG_ACK
            // Extended log with ACK/SR/CSI details
            // Calculate ranges (0=LSB, MSB is highest bit)
            int ack_msb = (pucch->n_harq > 0) ? (pucch->n_harq - 1) : -1;
            int ack_lsb = (pucch->n_harq > 0) ? 0 : -1;
            int sr_msb = (pucch->n_sr > 0) ? (pucch->n_harq + pucch->n_sr - 1) : -1;
            int sr_lsb = (pucch->n_sr > 0) ? pucch->n_harq : -1;
            
            // Extract components for verification
            uint64_t ack_extracted = (pucch->n_harq > 0) ? (pucch_pdu->payload & ((1UL << pucch->n_harq) - 1)) : 0;
            uint64_t sr_extracted = (pucch->n_sr > 0) ? ((pucch_pdu->payload >> pucch->n_harq) & ((1UL << pucch->n_sr) - 1)) : 0;
            
            // Generate bitstrings for each component
            char ack_msb_str[64] = {0}, ack_lsb_str[64] = {0};
            char sr_msb_str[64] = {0}, sr_lsb_str[64] = {0};
            char csi_msb_str[64] = {0}, csi_lsb_str[64] = {0};
            
            if (pucch->n_harq > 0) {
              get_bitstring_msb(ack_msb_str, sizeof(ack_msb_str), pucch->ack_payload, pucch->n_harq);
              get_bitstring_lsb(ack_lsb_str, sizeof(ack_lsb_str), pucch->ack_payload, pucch->n_harq);
            } else {
              snprintf(ack_msb_str, sizeof(ack_msb_str), "(none)");
              snprintf(ack_lsb_str, sizeof(ack_lsb_str), "(none)");
            }
            
            if (pucch->n_sr > 0) {
              get_bitstring_msb(sr_msb_str, sizeof(sr_msb_str), pucch->sr_payload, pucch->n_sr);
              get_bitstring_lsb(sr_lsb_str, sizeof(sr_lsb_str), pucch->sr_payload, pucch->n_sr);
            } else {
              snprintf(sr_msb_str, sizeof(sr_msb_str), "(none)");
              snprintf(sr_lsb_str, sizeof(sr_lsb_str), "(none)");
            }
            
            if (pucch->n_csi > 0) {
              get_bitstring_msb(csi_msb_str, sizeof(csi_msb_str), pucch->csi_part1_payload, pucch->n_csi);
              get_bitstring_lsb(csi_lsb_str, sizeof(csi_lsb_str), pucch->csi_part1_payload, pucch->n_csi);
            } else {
              snprintf(csi_msb_str, sizeof(csi_msb_str), "(none)");
              snprintf(csi_lsb_str, sizeof(csi_lsb_str), "(none)");
            }
            
            // Main log line with key info
            LOG_I(NR_MAC, "[RTD_MATCH] time=%s ts_us=%llu frame=%d slot=%d subframe=%d numerology=%d rnti=%d pucchRes=%d format=2 finalPayload=0x%lx\n",
                  wall, (unsigned long long)ts_us, frame, slot, subframe, numerology, rnti, pucch_res_id, pucch_pdu->payload);
            
            // Detail line with ACK/SR/CSI breakdown
            LOG_I(NR_MAC, "[RTD_MATCH_DETAIL] ackBits=%d ackHex=0x%lx ackMSB=%s ackLSB=%s ackRange=[%d:%d] ackExtracted=0x%lx "
                  "srBits=%d srHex=0x%lx srMSB=%s srLSB=%s srRange=[%d:%d] srExtracted=0x%lx "
                  "csiBits=%d csiHex=0x%lx csiMSB=%s csiLSB=%s csiRange=[%d:%d] csiExtracted=0x%lx "
                  "finalMSB=%s finalLSB=%s\n",
                  pucch->n_harq, pucch->ack_payload, ack_msb_str, ack_lsb_str, ack_msb, ack_lsb, ack_extracted,
                  pucch->n_sr, pucch->sr_payload, sr_msb_str, sr_lsb_str, sr_msb, sr_lsb, sr_extracted,
                  pucch->n_csi, pucch->csi_part1_payload, csi_msb_str, csi_lsb_str, csi_msb, csi_lsb, csi_extracted,
                  bits_msb, bits_lsb);
#else
            LOG_I(NR_MAC, "[RTD_MATCH] time=%s ts_us=%llu frame=%d slot=%d subframe=%d numerology=%d rnti=%d pucchRes=%d format=2 ackBits=%d srBits=%d csiBits=%d csiOffset=%d csiRange=[%d:%d] finalPayload=0x%lx csiExtracted=0x%lx bitsMSB=%s bitsLSB=%s\n",
                  wall, (unsigned long long)ts_us, frame, slot, subframe, numerology, rnti, pucch_res_id,
                  pucch->n_harq, pucch->n_sr, pucch->n_csi, csi_offset, csi_msb, csi_lsb,
                  pucch_pdu->payload, csi_extracted, bits_msb, bits_lsb);
#endif
          }
        }
#endif
      ...
      case NR_PUCCH_Resource__format_PR_format3 :
        ...
        // RTD_MATCH: Log final PUCCH payload assembly with timestamp and frame/slot info
        {
          ...
          if (clock_gettime(CLOCK_MONOTONIC, &ts) == 0) {
            ...
            // Main log line with key info
            LOG_I(NR_MAC, "[RTD_MATCH] time=%s ts_us=%llu frame=%d slot=%d subframe=%d numerology=%d rnti=%d pucchRes=%d format=3 finalPayload=0x%lx\n",
                  wall, (unsigned long long)ts_us, frame, slot, subframe, numerology, rnti, pucch_res_id, pucch_pdu->payload);
            ...
            LOG_I(NR_MAC, "[RTD_MATCH_DETAIL] ackBits=%d ackHex=0x%lx ackMSB=%s ackLSB=%s ackRange=[%d:%d] ackExtracted=0x%lx "
                  ...
                  bits_msb, bits_lsb);
#else
            LOG_I(NR_MAC, "[RTD_MATCH] time=%s ts_us=%llu frame=%d slot=%d subframe=%d numerology=%d rnti=%d pucchRes=%d format=3 ackBits=%d srBits=%d csiBits=%d csiOffset=%d csiRange=[%d:%d] finalPayload=0x%lx csiExtracted=0x%lx bitsMSB=%s bitsLSB=%s\n",
                  wall, (unsigned long long)ts_us, frame, slot, subframe, numerology, rnti, pucch_res_id,
                  pucch->n_harq, pucch->n_sr, pucch->n_csi, csi_offset, csi_msb, csi_lsb,
                  pucch_pdu->payload, csi_extracted, bits_msb, bits_lsb);
#endif
      ...
      case NR_PUCCH_Resource__format_PR_format4 :
        ...
        // RTD_MATCH: Log final PUCCH payload assembly with timestamp and frame/slot info
        {
          ...
          if (clock_gettime(CLOCK_MONOTONIC, &ts) == 0) {
            ...
            // Main log line with key info
            LOG_I(NR_MAC, "[RTD_MATCH] time=%s ts_us=%llu frame=%d slot=%d subframe=%d numerology=%d rnti=%d pucchRes=%d format=4 finalPayload=0x%lx\n",
                  wall, (unsigned long long)ts_us, frame, slot, subframe, numerology, rnti, pucch_res_id, pucch_pdu->payload);
            ...
            LOG_I(NR_MAC, "[RTD_MATCH_DETAIL] ackBits=%d ackHex=0x%lx ackMSB=%s ackLSB=%s ackRange=[%d:%d] ackExtracted=0x%lx "
                  ...
                  bits_msb, bits_lsb);
#else
            LOG_I(NR_MAC, "[RTD_MATCH] time=%s ts_us=%llu frame=%d slot=%d subframe=%d numerology=%d rnti=%d pucchRes=%d format=4 ackBits=%d srBits=%d csiBits=%d csiOffset=%d csiRange=[%d:%d] finalPayload=0x%lx csiExtracted=0x%lx bitsMSB=%s bitsLSB=%s\n",
                  wall, (unsigned long long)ts_us, frame, slot, subframe, numerology, rnti, pucch_res_id,
                  pucch->n_harq, pucch->n_sr, pucch->n_csi, csi_offset, csi_msb, csi_lsb,
                  pucch_pdu->payload, csi_extracted, bits_msb, bits_lsb);
#endif
...
#ifdef RTD_MATCH_LOG
              // RTD_MATCH: Log CSI payload generation with timestamp and frame/slot info
              {
                struct timespec ts;
                if (clock_gettime(CLOCK_MONOTONIC, &ts) == 0) {
                  uint64_t ts_us = (uint64_t)ts.tv_sec * 1000000ULL + (uint64_t)ts.tv_nsec / 1000ULL;
                  ...
                  // Generate bitstring (MSB->LSB: b12->b0)
                  char bits_msb[64] = {0};
                  for (int i = csi.p1_bits - 1; i >= 0; i--) {
                    bits_msb[csi.p1_bits - 1 - i] = ((csi.part1_payload >> i) & 1) ? '1' : '0';
                  }
                  
                  // Generate bitstring (LSB->MSB: b0->b12)
                  char bits_lsb[64] = {0};
                  for (int i = 0; i < csi.p1_bits; i++) {
                    bits_lsb[i] = ((csi.part1_payload >> i) & 1) ? '1' : '0';
                  }
    ...
    if (csi_resourceconfig->csi_ResourceConfigId == csi_ResourceConfigId) {
      ...
      for (int csi_idx = 0; csi_idx < csi_MeasConfig->nzp_CSI_RS_ResourceSetToAddModList->list.count; csi_idx++) {
        if (csi_MeasConfig->nzp_CSI_RS_ResourceSetToAddModList->list.array[csi_idx]->nzp_CSI_ResourceSetId ==
          ...
          // Format (MSB → LSB): RI(2) | PMI_i2(2) | PMI_i11(4) | CQI(4) | PAD(1) = 13 bits
