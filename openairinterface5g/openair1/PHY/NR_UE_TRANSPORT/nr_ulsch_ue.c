...
// Function to lookup beta offset value for CSI from Table 9.3-1 in TS 38.213
// CSI uses the same beta offset table as HARQ-ACK
static double get_beta_offset_csi(uint8_t beta_offset_index)
{
  return get_beta_offset_harq_ack(beta_offset_index);
}
...
static rate_match_info_uci_t calc_rate_match_info_uci(const nfapi_nr_ue_pusch_pdu_t *pusch_pdu,
                                                      ...
                                                      unsigned int *G)
{
  // get alpha scaling value (common for all UCI types)
  ...
  uint32_t G_remaining = *G;

  // Process HARQ-ACK if present
  ...
  if (oack > 0) {
    ...
    if (oack_rvd == 0) {
      G_remaining -= rminfo.E_uci_ACK;
    }
  }

  // Process CSI part1 if present
  uint16_t ocsi1 = pusch_pdu->pusch_uci.csi_part1_bit_length;
  if (ocsi1 > 0) {
    uint8_t beta_offset_index = pusch_pdu->pusch_uci.beta_offset_csi1;
    double beta = get_beta_offset_csi(beta_offset_index);
    
    // Calculate Q_dash for CSI part1, TS 38.212 section 6.3.2.4.2
    // For CSI part1, we use the same formula as HARQ-ACK but with CSI beta offset
    rminfo.Q_dash_CSI1 = get_Qd(ocsi1, beta, alpha, sumKr, s1, s2);
    rminfo.E_uci_CSI1 = rminfo.Q_dash_CSI1 * nlqm;
    G_remaining -= rminfo.E_uci_CSI1;
  }

  // Process CSI part2 if present
  uint16_t ocsi2 = pusch_pdu->pusch_uci.csi_part2_bit_length;
  if (ocsi2 > 0) {
    uint8_t beta_offset_index = pusch_pdu->pusch_uci.beta_offset_csi2;
    double beta = get_beta_offset_csi(beta_offset_index);
    
    // Calculate Q_dash for CSI part2, TS 38.212 section 6.3.2.4.2
    rminfo.Q_dash_CSI2 = get_Qd(ocsi2, beta, alpha, sumKr, s1, s2);
    rminfo.E_uci_CSI2 = rminfo.Q_dash_CSI2 * nlqm;
    G_remaining -= rminfo.E_uci_CSI2;
  }

  rminfo.G_ulsch = G_remaining;
  *G = rminfo.G_ulsch;
                            ...
                            bool was_symbol_used[NR_NUMBER_OF_SYMBOLS_PER_SLOT])
{
  ...
  // Calculate rate matching info for all UCI types (HARQ-ACK, CSI part1, CSI part2)
  ...
  // Process UCI multiplexing (HARQ-ACK, CSI part1, CSI part2)
  bool has_uci = (pusch_pdu->pusch_uci.harq_ack_bit_length != 0 ||
                  pusch_pdu->pusch_uci.csi_part1_bit_length != 0 ||
                  pusch_pdu->pusch_uci.csi_part2_bit_length != 0);

  if (has_uci) {
    LOG_D(PHY, "[UCI_ON_PUSCH] Initial G: %u\n", G_initial_total_pusch_bits);
    LOG_D(PHY, "[UCI_ON_PUSCH] HARQ-ACK: %u bits, CSI part1: %u bits, CSI part2: %u bits\n",
          pusch_pdu->pusch_uci.harq_ack_bit_length,
          pusch_pdu->pusch_uci.csi_part1_bit_length,
          pusch_pdu->pusch_uci.csi_part2_bit_length);

    if (pucch_pdu == NULL) {
      LOG_E(PHY, "nr_ue_ulsch_procedures: pucch_pdu is NULL but UCI is present. Cannot proceed with UCI encoding.\n");
      stop_meas_nr_ue_phy(UE, PUSCH_PROC_STATS);
      return;
    }

    uint8_t temp_codeword[G_initial_total_pusch_bits];
    memset(temp_codeword, 0, G_initial_total_pusch_bits);
    uint64_t b_ack[16] = {0}; // limit to 1024-bit encoded length
    uint64_t b_csi1[16] = {0};
    uint64_t b_csi2[16] = {0};

    // Encode HARQ-ACK if present
    if (pusch_pdu->pusch_uci.harq_ack_bit_length != 0) {
      LOG_D(PHY, "[UCI_ON_PUSCH] Encoding HARQ-ACK: %u bits -> %u bits\n",
            pusch_pdu->pusch_uci.harq_ack_bit_length, rm_info.E_uci_ACK);
    ...
    // Encode CSI part1 if present
    if (pusch_pdu->pusch_uci.csi_part1_bit_length != 0) {
      LOG_D(PHY, "[UCI_ON_PUSCH] Encoding CSI part1: %u bits -> %u bits\n",
            pusch_pdu->pusch_uci.csi_part1_bit_length, rm_info.E_uci_CSI1);
    ...
    // Encode CSI part2 if present
    if (pusch_pdu->pusch_uci.csi_part2_bit_length != 0) {
      LOG_D(PHY, "[UCI_ON_PUSCH] Encoding CSI part2: %u bits -> %u bits\n",
            pusch_pdu->pusch_uci.csi_part2_bit_length, rm_info.E_uci_CSI2);
    ...
    LOG_D(PHY,
          "[UCI_ON_PUSCH] G_ulsch=%u (updated G[pusch_id]), E_ACK=%u, E_ACK_rvd=%u, E_CSI1=%u, E_CSI2=%u, total_len=%u\n",
          G[pusch_id],
          rm_info.E_uci_ACK,
    ...
    // Map CSI part1 if present (similar to HARQ-ACK mapping)
    // Note: According to TS 38.212, CSI part1 and part2 should be mapped after HARQ-ACK
    // For now, we use a simplified mapping similar to HARQ-ACK
    if (pusch_pdu->pusch_uci.csi_part1_bit_length != 0 && rm_info.E_uci_CSI1 > 0) {
      // Append CSI part1 to the codeword after HARQ-ACK
      uint32_t csi1_start_pos = rm_info.E_uci_ACK + (rm_info.E_uci_ACK_rvd > 0 ? rm_info.E_uci_ACK_rvd : 0);
      for (uint32_t i = 0; i < rm_info.E_uci_CSI1 && (csi1_start_pos + i) < G_initial_total_pusch_bits; i++) {
        uint32_t word_idx = i / 64;
        uint32_t bit_in_word_idx = i % 64;
        uint8_t bit = (b_csi1[word_idx] >> bit_in_word_idx) & 1;
        if (temp_codeword[csi1_start_pos + i] == 0) { // Only overwrite if not already set by ULSCH
          temp_codeword[csi1_start_pos + i] = bit;
        }
      }
      LOG_D(PHY, "[UCI_ON_PUSCH] Mapped CSI part1: %u bits starting at position %u\n", rm_info.E_uci_CSI1, csi1_start_pos);
    }
    
    // Map CSI part2 if present (after CSI part1)
    if (pusch_pdu->pusch_uci.csi_part2_bit_length != 0 && rm_info.E_uci_CSI2 > 0) {
      uint32_t csi2_start_pos = rm_info.E_uci_ACK + (rm_info.E_uci_ACK_rvd > 0 ? rm_info.E_uci_ACK_rvd : 0) + rm_info.E_uci_CSI1;
      for (uint32_t i = 0; i < rm_info.E_uci_CSI2 && (csi2_start_pos + i) < G_initial_total_pusch_bits; i++) {
        uint32_t word_idx = i / 64;
        uint32_t bit_in_word_idx = i % 64;
        uint8_t bit = (b_csi2[word_idx] >> bit_in_word_idx) & 1;
        if (temp_codeword[csi2_start_pos + i] == 0) { // Only overwrite if not already set by ULSCH
          temp_codeword[csi2_start_pos + i] = bit;
        }
      }
      LOG_D(PHY, "[UCI_ON_PUSCH] Mapped CSI part2: %u bits starting at position %u\n", rm_info.E_uci_CSI2, csi2_start_pos);
    }

    stop_meas_nr_ue_phy(UE, UCI_ON_PUSCH_MAPPING);
    
    // Copy the final codeword if any UCI was processed
    if (has_uci) {
